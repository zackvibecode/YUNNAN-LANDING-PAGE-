        ...
        <details class="group bg-surface-container-lowest border border-outline-variant/30 rounded-2xl overflow-hidden hover:border-primary/50 hover:shadow-lg transition-all shadow-sm">
            <summary class="list-none flex justify-between items-center p-6 cursor-pointer font-extrabold text-xl text-on-surface select-none">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <span class="block text-primary text-xl">Musim Musim Sejuk & Bunga</span>
...
const premierRegex = /<div id="premierTab" class="tab-content active">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/;
const promoRegex = /<div id="promoTab" class="tab-content">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/;
...
if(html.match(promoRegex)) {
    ...
    // try fallback regex 
    const fallbackPromoRegex = /<div id="promoTab" class="tab-content">[\s\S]*?(?=<\/section>)/;
    html = html.replace(fallbackPromoRegex, promoHTML + "\n</div>");
}

fs.writeFileSync('index.html', html);
console.log('Update Phase 5 (Exact PDF Dates & Hover Smooth) completed');
